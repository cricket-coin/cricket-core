...
pkgdatadir = $(datadir)/crk
pkgincludedir = $(includedir)/crk
pkglibdir = $(libdir)/crk
pkglibexecdir = $(libexecdir)/crk
...
CONFIG_HEADER = $(top_builddir)/src/config/crk-config.h
...
am__DIST_COMMON = $(srcdir)/Makefile.in \
	...
	$(top_srcdir)/src/config/crk-config.h.in \
...
ACLOCAL = ${SHELL} /root/masternode/cricket-core/build-aux/missing aclocal-1.15
AMTAR = $${TAR-tar}
AM_DEFAULT_VERBOSITY = 0
AR = /usr/bin/ar
AUTOCONF = ${SHELL} /root/masternode/cricket-core/build-aux/missing autoconf
AUTOHEADER = ${SHELL} /root/masternode/cricket-core/build-aux/missing autoheader
AUTOMAKE = ${SHELL} /root/masternode/cricket-core/build-aux/missing automake-1.15
AWK = gawk
BDB_CPPFLAGS = 
...
LUPDATE = 
MAINT = 
MAKEINFO = ${SHELL} /root/masternode/cricket-core/build-aux/missing makeinfo
MAKENSIS = 
MANIFEST_TOOL = :
MINIUPNPC_CPPFLAGS = 
MINIUPNPC_LIBS = 
...
OTOOL64 = 
PACKAGE = crk
PACKAGE_BUGREPORT = cricket-coin.com
PACKAGE_NAME = Cricket Core
PACKAGE_STRING = Cricket Core 1.0.0
PACKAGE_TARNAME = crk
PACKAGE_URL = 
...
ZMQ_CFLAGS = 
ZMQ_LIBS = -lzmq
abs_builddir = /root/masternode/cricket-core
abs_srcdir = /root/masternode/cricket-core
abs_top_builddir = /root/masternode/cricket-core
abs_top_srcdir = /root/masternode/cricket-core
ac_ct_AR = ar
ac_ct_CC = gcc
ac_ct_CXX = g++
ac_ct_DUMPBIN = 
...
install_sh = ${SHELL} /root/masternode/cricket-core/build-aux/install-sh
...
SUBDIRS = src
GZIP_ENV = "-9n"
BITCOIND_BIN = $(top_builddir)/src/crkd$(EXEEXT)
BITCOIN_QT_BIN = $(top_builddir)/src/qt/crk-qt$(EXEEXT)
BITCOIN_CLI_BIN = $(top_builddir)/src/crk-cli$(EXEEXT)
BITCOIN_WIN_INSTALLER = $(PACKAGE)-$(PACKAGE_VERSION)-win$(WINDOWS_BITS)-setup$(EXEEXT)
empty := 
space := $(empty) $(empty)
OSX_APP = Cricket-Qt.app
...
COVERAGE_INFO = baseline_filtered_combined.info baseline.info block_test.info \
  leveldb_baseline.info test_crk_filtered.info total_coverage.info \
  baseline_filtered.info block_test_filtered.info \
  leveldb_baseline_filtered.info test_crk_coverage.info test_crk.info

OSX_APP_BUILT = $(OSX_APP)/Contents/PkgInfo $(OSX_APP)/Contents/Resources/empty.lproj \
  $(OSX_APP)/Contents/Resources/bitcoin.icns $(OSX_APP)/Contents/Info.plist \
  $(OSX_APP)/Contents/MacOS/Cricket-Qt $(OSX_APP)/Contents/Resources/Base.lproj/InfoPlist.strings
...
src/config/crk-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/crk-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/crk-config.h
$(top_srcdir)/src/config/crk-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/crk-config.h src/config/stamp-h1
...
$(OSX_APP)/Contents/MacOS/Cricket-Qt: $(BITCOIN_QT_BIN)
	$(MKDIR_P) $(@D)
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/Cricket-Qt
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/Cricket-Qt: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#test_crk.info: baseline_filtered_combined.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c -d $(abs_builddir)/src -t test_crk -o $@
#	$(LCOV) -z -d $(abs_builddir)/src
#	$(LCOV) -z -d $(abs_builddir)/src/leveldb

#test_crk_filtered.info: test_crk.info
#	$(LCOV) -r $< "/usr/include/*" -o $@

#block_test.info: test_crk_filtered.info
#	$(MKDIR_P) qa/tmp
...
#test_crk_coverage.info: baseline_filtered_combined.info test_crk_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_crk_filtered.info -o $@

#total_coverage.info:  baseline_filtered_combined.info test_crk_filtered.info block_test_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_crk_filtered.info -a block_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_crk.coverage/.dirstamp:  test_crk_coverage.info
#	$(GENHTML) -s $< -o $(@D)
#	@touch $@
...
#cov: test_crk.coverage/.dirstamp total.coverage/.dirstamp

# Cricket: disable the outdated comparison tool test
...
clean-local:
	rm -rf test_crk.coverage/ total.coverage/ $(OSX_APP)
