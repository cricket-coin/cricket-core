    ...
    if (fMasterNode) {
        ...
        BOOST_FOREACH (const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat Cricket and make it unusable. To
                stop these kinds of attacks 1 in 10 successful transactions are charged. This
                adds up to a cost of 0.001 CRK per transaction on average.
            */
            if (r <= 10) {
...
bool CPrivatesendPool::DoAutomaticDenominating(bool fDryRun)
{
    ...
    CAmount nBalanceNeedsAnonymized = nAnonymizeCrkAmount * COIN - pwalletMain->GetAnonymizedBalance();
...
void CPrivatesendPool::GetDenominationsToString(int nDenom, std::string& strDenom)
{
    ...
    //
    // bit 0 - 100CRK+1 ( bit on if present )
    // bit 1 - 10CRK+1
    // bit 2 - 1CRK+1
    // bit 3 - .1CRK+1
    // bit 3 - non-denom
