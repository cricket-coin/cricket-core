Cricket Core version 2.3.0 is now available from:
...
[
  {
    ...
    winner: {
        address: addr,    (string) Cricket MN Address,
...
[
  {
    ...
    "addr": addr,      (string) Masternode Cricket address
...
Cricket Core now enables bloom filters by default to support SPV clients like mobile wallets. This feature can be disabled by using the `-peerbloomfilters` option on startup.
...
- #195 `aee05fe` [ZMQ] ZMQ integration for Cricket (Mrs-X)
...
### Tests and QA
- #191 `3a778c3` [Tests] Fix the unit test suite for use with Cricket (Fuzzbawls)
